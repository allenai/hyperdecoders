        ...
        def detect_answer(sample, offsets_chunk):
            for i, span in enumerate(sample["detected_answers"]["char_spans"]):
                for start, end in zip(
                    span["start"], span["end"]
                ):  # we can have multiple answer instances
...
def chunk_dataset(tokenizer, dataset, stride=128, max_length=512, filter_nulls=False):
    for sample in dataset:
        for chunked_sample in chunk_sample(
            tokenizer, sample, stride, max_length, filter_nulls
        ):
...
if __name__ == "__main__":
    ...
    chunked_ds = list(
        chunk_dataset(tokenizer, mrqa, stride=128, max_length=512, filter_nulls=True)
    )
