"""Evaluation script for MRQA
Adapted from Official evaluation script for the MRQA Workshop Shared Task.
...
def evaluate(answers, predictions, skip_no_answer=False):
    ...
    for qid, ground_truths in answers.items():
        if qid not in predictions:
            continue
        total += 1
        # filter out null guesses
        filtered_preds = [pred for pred in predictions[qid] if pred[0] != '']
        if filtered_preds == []:
            prediction = ''
        else:
            probs = [pred[1] for pred in filtered_preds]
            max_prob_index = probs.index(min(probs))
            prediction = filtered_preds[max_prob_index][0]
